...
let mapi f s = init (length s) (fun i -> f i (get s i))

let fold f s accu =
    let n = length s in
    let rec loop i accu =
        if i = n then accu else
        loop (i + 1) (f (get s i) accu) in
    loop 0 accu
...
let foldi16 f s accu =
    ...
    let rec loop i accu =
        if i = n then accu else
        loop (i + 1) (f i (get16 s i) accu) in
    loop 0 accu

let iter f s = for i = 0 to length s - 1 do f (get s i) done
let iteri f s = for i = 0 to length s - 1 do f i (get s i) done
let iteri8 f s = for i = 0 to length8 s - 1 do f i (get8 s i) done
let iteri16 f s = for i = 0 to length16 s - 1 do f i (get16 s i) done
...
let to_string s =
    ...
    String.init (length s) (fun i -> if get s i then '1' else '0')
...
let to_bin_string ?n s =
    let ns = length s in
    let put_octet i = Char.chr (if 8 * i < ns then get8 s i else 0) in
...
let to_hex_string ?n s =
    ...
    let put_quartet i =
        let x = if (4 * i) >= ns then 0 else
                get8 s (i / 2) lsr (4*(1 - i mod 2)) land 15 in
