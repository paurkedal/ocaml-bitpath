...
let lower_half = function
 ...
 | Y (s, _) -> s
 | P (p, s) ->
    if Bitpath.get p 0 then Bot else
...
let upper_half = function
 ...
 | Y (_, s) -> s
 | P (p, s) ->
    if not (Bitpath.get p 0) then Bot else
    unzoom (Bitpath.suffix 1 p) s
...
let rec isecn sL sR = match sL, sR with
 ...
 | P (pL, sL), _ ->
    (match zoom pL sR with
     | Bot -> Bot
     | sR' -> unzoom pL (isecn sL sR'))
 | _, P (pR, sR) ->
    (match zoom pR sL with
     | Bot -> Bot
...
let rec rel_compl sC sR = match sC, sR with
 ...
 | Bot, _ -> sR
 | _, Top -> abs_compl sC
 | Y (sC0, sC1), Y (sR0, sR1) -> appose (rel_compl sC0 sR0) (rel_compl sC1 sR1)
